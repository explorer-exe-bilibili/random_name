...
int mode = 1, step = 0, bottom1x, bottom1y,starcount;
...
void printstars(HDC hdc, int number) {
	...
	case 6: {
		...
		int x, y;
		for (int i = 500; i > 0; i--) {
			x = getname::randomIntegerBetween(0, mywindows::windowWidth * 0.99);
			y = getname::randomIntegerBetween(0, mywindows::windowHeight * 0.99);
