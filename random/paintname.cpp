...
void paintname::showname1() {
	...
	if (!setting::offvideo) {
		if (is4star == 0 AND is5star == 0)
			directshow::play(L"\\files\\video\\3star-single.mp4");
		if (is4star == 1 AND is5star == 0)
			directshow::play(L"\\files\\video\\4star-single.mp4");
		if (is5star == 1)
			directshow::play(L"\\files\\video\\5star-single.mp4");
	}
	if (!setting::offmusic)
...
void paintname::showname10() {
	...
	if (!setting::offvideo) {
		if (is5star == 1)
			directshow::play(L"\\files\\video\\5star-multi.mp4");
		else if (is5star == 0)
			directshow::play(L"\\files\\video\\4star-multi.mp4");
	}
	if (!setting::offmusic)
			mciSendString(L"stop bgm", NULL, 0, NULL);
	is5star = 0;
